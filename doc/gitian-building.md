...
*Setup instructions for a Gitian build of Innoket Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Innoket
...
- [Building Innoket Core](#building-innoket-core)
...
Clone the git repositories for Innoket Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/innoketcoin/innoket
```
...
in the Innoket Core repository under 'Fetch and build inputs' to install sources which require
...
To build Innoket Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Innoket Core repository.
...
```bash
    ...
    From https://github.com/innoketcoin/innoket
...
and the innoket git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../innoket/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/innoket.git
SIGPATH=/some/root/path/innoket-detached-sigs.git

./bin/gbuild --url innoket=${BTCPATH},signature=${SIGPATH} ../innoket/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/innoket-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/innoket-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/innoket-osx-build.assert
```
...
[innoket/gitian.sigs](https://github.com/innoketcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
