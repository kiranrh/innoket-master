...
// ***TODO*** ScanHash is not yet used in Innoket
//
...
void static BitcoinMiner(const CChainParams& chainparams)
{
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("InnoketMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("InnoketMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("InnoketMiner -- runtime error: %s\n", e.what());
        return;
    }
}
