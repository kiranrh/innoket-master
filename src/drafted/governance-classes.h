 ...
 *
 *   // network
 *   CInnoketNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.innoket.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CInnoketNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CInnoketNetwork(UniValue objIn)
    {
...
// // can be under: InnoketNetwork
// //   -- signature requirements : Key1(User)
// class CInnoketNetworkVariable : public CGovernanceObject
// {
// private:
