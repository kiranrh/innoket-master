...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current Innoket address for receiving payments to this account.\n"
            "\nArguments:\n"
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 7)
        throw runtime_error(
            "sendtoaddress \"innoketaddress\" amount ( \"comment\" \"comment-to\" subtractfeefromamount use_is use_ps )\n"
            ...
            "                             The recipient will receive less amount of Innoket than you enter in the amount field.\n"
...
UniValue instantsendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "instantsendtoaddress \"innoketaddress\" amount ( \"comment\" \"comment-to\" subtractfeefromamount )\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a innoket address."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 7)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The innoket address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less innokets than you enter in their corresponding amount field.\n"
    ...
    BOOST_FOREACH(const string& name_, keys)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Innoket address: ")+name_);
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Wallet encrypted; Innoket Core server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
